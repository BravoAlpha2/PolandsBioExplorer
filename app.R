  ...
  dashboardHeader(
    ...
    <div class='topbar-brand'>
      ...
      <div class='topbar-brand-copy'>
        <div class='topbar-title'>Poland's BioExplorer</div>
        <div class='topbar-subtitle'>Spatial and temporal biodiversity exploration</div>
      </div>
    </div>
  ")
  ),
...
server <- function(input, output, session) {
  ...
  selected_species_data <- reactive({
    ...
    filter_observation_data(
      dat = dat,
      year_range = search_state$year_range(),
      only_with_media = search_state$only_with_media()
    )
  })
  ...
  current_timeline_data <- reactive({
    if (current_mode() == "default") {
      ...
      build_timeline_data(selected_species_data())
    }
  })
